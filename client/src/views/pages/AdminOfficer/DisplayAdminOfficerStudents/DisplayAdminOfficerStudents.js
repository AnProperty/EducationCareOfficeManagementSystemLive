...
const DisplayAdminOfficerStudents = ({ applicantStudent, setApplicantStudent }) => {
  const apllicant = JSON.parse(localStorage.getItem('user'))
  console.log('object', applicantStudent)
  ...
  const [status, setStatus] = useState('application-processing')
  ...
  const handleFilter = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/utilities/filter-student-list/${apllicant.employee_id}?${query.toString()}`,
      )
      setApplicantStudent(response.data.data)
    } catch (error) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/utilities/download-leads/${apllicant.employee_id}?${query.toString()}`,
        {
          responseType: 'blob',
        },
      )
  ...
  const handleFullNameChange = async (e) => {
    ...
    if (value) {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_BASE_URL}/utilities/get-student-suggestion/${apllicant.employee_id}?fullName=${value}`,
        )
  ...
  const handleDelete = async (studentId) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_API_BASE_URL}/utilities/delete-student/${studentId}/${apllicant._id}`,
        )
        setApplicantStudent(applicantStudent.filter((student) => student._id !== studentId)) // Update list
      } catch (error) {
          ...
          {paginatePaper?.map((item, index) => {
            return (
              <CTableRow v-for="item in tableItems" key={index} className="text-center">
                ...
                <CTableDataCell>
                  <Link
                    to={`/admin-officer/student-details/${item.studentId}/${item.counselor.employee_id}`}
                    state={{ item: item }}
                  >
                    <button className="button btn btn3">More Info</button>
                  </Link>
                </CTableDataCell>
                <CTableDataCell>
