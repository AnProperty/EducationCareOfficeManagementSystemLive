...
const DisplayCounselorFollowUpStudents = () => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API_BASE_URL}/utilities/filter-student-list/${counselor.employee_id}?status=follow-up`,
    )
      .then((res) => res.json())
  ...
  const handleDelete = async (studentId) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_API_BASE_URL}/utilities/delete-student/${studentId}/${counselor._id}`,
        )
        setCounselorEnrolledStudent(
          counselorEnrolledStudent.filter((student) => student._id !== studentId),
        ) // Update list
      } catch (error) {
          ...
          {counselorEnrolledStudent?.map((item, index) => {
            return (
              <CTableRow v-for="item in tableItems" key={index} className="text-center">
                ...
                <CTableDataCell>
                  <Link
                    to={`/counselor/student-details/${item.studentId}/${counselor.employee_id}`}
                    state={{ item: item }}
                  >
