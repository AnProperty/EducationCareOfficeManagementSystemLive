...
const DisplayVisaStudentsList = ({ VisaTeamStudent, setVisaTeamStudent }) => {
  ...
  const handleDelete = async (studentId) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_API_BASE_URL}/utilities/delete-student/${studentId}/${visaempl._id}`,
        )
        setVisaTeamStudent(VisaTeamStudent.filter((student) => student._id !== studentId)) // Update list
      } catch (error) {
