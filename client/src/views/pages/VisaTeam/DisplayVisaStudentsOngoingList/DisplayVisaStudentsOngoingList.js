...
const DisplayVisaStudentsOngoingList = () => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API_BASE_URL}/utilities/filter-student-list/${applicant.employee_id}?status=visa-processing`,
    )
      .then((res) => res.json())
          ...
          {applicantOngoingStudent?.map((item, index) => {
            return (
              <CTableRow v-for="item in tableItems" key={index} className="text-center">
                ...
                <CTableDataCell>
                  <Link
                    to={`/visa-process/student-details/${item.studentId}/${applicant.employee_id}`}
                    state={{ item: item }}
                  >
