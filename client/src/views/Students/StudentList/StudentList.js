...
const StudentList = () => {
  ...
  const handleDownload = async () => {
    try {
      ...
      const finalEndDate = endDate || getNextDayDate()
      query.append('endDate', finalEndDate)
      if (fullName) query.append('fullName', fullName) // Add fullName to the download query

      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/utilities/download-leads/${status}?${query.toString()}`,
        {
          responseType: 'blob',
        },
      )
  ...
  return (
    <>
      <section>
        <div className="d-flex my-4 justify-content-space-evenly forDownloadLabel">
          ...
          <label>Full Name:</label>
            <div className='relative flex-1'>
              ...
              {suggestions.length > 0 && (
                <ul className="suggestions-list absolute bg-white border border-gray-300 w-full rounded mt-1 max-h-40 overflow-y-auto z-20">
